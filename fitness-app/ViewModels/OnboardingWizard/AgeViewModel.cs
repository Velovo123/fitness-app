...
public class AgeViewModel: OnboardingWizardBaseViewModel
{
    ...
    {
        ItemSelectedCommand = new Command<(int Component, int Row, IList<int> ItemIndexes)>(OnItemSelected);
        MoveNextCommand = CreateAsyncCommand(OnMoveNextAsync);
    }
    
    private void OnItemSelected((int Component, int Row, IList<int> ItemIndexes) tuple)
    {
        var value = Days[tuple.Row]?.ToString();
