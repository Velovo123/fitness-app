...
using User = fitness_app.Models.User;
...
public class UserFitnessDataService : IUserFitnessDataService
    {
        ...
        public async Task<UserFitnessData?> UpdateUserFitnessDataAsync(Session session, Models.User user)
        {
            var currentRecord = await GetUserFitnessDataAsync(session);
            if (currentRecord == null)
            {
            ...
            var newData = ConvertUserToUserFitnessData(user);
            
            MergeUserFitnessData(newData, currentRecord);

            var updateResponse = await _supabaseClient
                .From<UserFitnessData>()
                .Filter("id", Supabase.Postgrest.Constants.Operator.Equals, GetUserId(session))
                .Update(currentRecord);
        ...
        private UserFitnessData ConvertUserToUserFitnessData(Models.User user)
        {
            return new UserFitnessData
            {
                // Here we map the relevant fitness data properties.
                // Adjust the mapping based on the properties that are shared between your models.
